...
# ── Flet data ─────────────────────────────────────────────────────────────────
def make_flets(spread, vel, n, generation):
    flets = []
    for i in range(n):
        ang = i * math.pi * 2 / n
...
def step_flets(fs, t, log_fn):
    ...
    for f in nf:
        ...
        if f['role'] in ('LAUNCH', 'APPROACH'):
            ...
            ddx = tx - f['x'];  ddy = ty - f['y']
            dd  = math.hypot(ddx, ddy)
            if dd > 1:
                s = 0.12 / dd
                f['vx'] += ddx * s;  f['vy'] += ddy * s
            f['vx'] *= 0.97;  f['vy'] *= 0.97
            f['energy'] = max(0.0, f['energy'] - 0.0008)
            if t > 30:
                f['role'] = 'APPROACH'
            if dist < HUB_R + DOCK_R:
                f['role'] = 'NEGOTIATE'
...
class FletSim:
    ...
    def _draw_panel(self):
        ...
        ax.text(0.05, 1.12, 'FLET STATUS', fontsize=8, color=TEXT_DIM,
